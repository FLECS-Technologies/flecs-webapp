...
import { instancenicconfig } from '@shared/components/help/helplinks';
...
const NetworkConfigTab: React.FC<NetworkConfigTabProps> = ({ instanceId, onChange }) => {
  ...
  const fetchNetworks = async () => {
    try {
      ...
      const networkAdaptersResponse = await api.system.systemNetworkAdaptersGet();
      const networkAdapters = networkAdaptersResponse.data.map((adapter: NetworkAdapter) => ({
        id: adapter.name,
        name: adapter.name,
  ...
  const handleNetworkActivationChange = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>, id: string, name: string): Promise<void> => {
      ...
      try {
        if (isActivated) {
          ...
          await api.instances.instancesInstanceIdConfigNetworksPost(instanceId, {
        ...
        setNetworks((prevNetworks) =>
          prevNetworks.map((network) =>
            network.id === id ? { ...network, is_activated: isActivated } : network,
          ),
        );
