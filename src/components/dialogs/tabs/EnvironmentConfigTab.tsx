...
const EnvironmentConfigTab: React.FC<EnvironmentConfigTabProps> = ({
  instanceId,
  onChange
}) => {
  ...
  function handleChange(index: number, key: string, value: string): void {
    setEnvVars((prev) =>
      prev.map((env, i) => (i === index ? { ...env, [key]: value } : env))
    )
  }
  ...
  return (
    <Box>
      ...
      <List>
        {envVars.length === 0 && (
          <Typography>No environment variables configured.</Typography>
        )}
