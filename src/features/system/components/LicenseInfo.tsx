...
function LicenseInfo() {
  ...
  const {
    ...
    isLoading: loading,
    isError: error,
  } = useGetDeviceLicenseInfo({ query: { queryKey: [`/device/license/info`, activated] } });
        ...
        <tbody>
          {info.license && (
            <tr>
              ...
              <td className="px-4 py-2">{info.license}</td>
            </tr>
          )}
          {info.sessionId?.id && (
            <tr>
              <td className="px-4 py-2">Session ID</td>
              <td className="px-4 py-2">{info.sessionId.id}</td>
            </tr>
          )}
          {info.sessionId?.timestamp && (
            <tr>
              <td className="px-4 py-2">Last session renewal</td>
              <td className="px-4 py-2">{String(info.sessionId.timestamp)}</td>
            </tr>
          )}
        </tbody>
      )}
