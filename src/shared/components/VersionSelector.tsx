...
import { findVersionByProperty, getLatestVersion } from '@shared/utils/version-utils';
  ...
  return (
    <Box sx={{ mt: 1, mb: 1 }}>
      ...
      {selectedVersion?.release_notes && (
        <Button size="small" onClick={onReleaseNoteButtonClick} startIcon={<ExternalLink size={16} />}>
          Release Notes
        </Button>
      )}
      {selectedVersion?.breaking_changes && (
        <Button size="small" onClick={onBreakingChangesButtonClick} startIcon={<ExternalLink size={16} />}>
          Breaking Changes
        </Button>
      )}
    </Box>
