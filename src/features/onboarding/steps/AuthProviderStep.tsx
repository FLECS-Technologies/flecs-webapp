...
const checkAuthProviderConfigured = async () =>
  extractCoreProviderId(unwrapSuccess(await getProvidersAuth())!) !== null;
...
const AuthProviderStepComponent: React.FC<{
  onNext?: () => void;
  onPrevious?: () => void;
  onComplete?: () => void;
  isLoading?: boolean;
  error?: string;
}> = ({ onNext, onPrevious, onComplete, isLoading: parentLoading, error: parentError }) => {
  ...
  const getProviderIds = (data: AuthProvidersAndDefaults | null) =>
    data?.providers ? Object.keys(data.providers) : [];
  const completeStep = async () => {
    await onComplete?.();
    onNext?.();
  };
  const configureAndComplete = async (id: string) => {
    setIsLoading(true);
    resetError();
    try {
      await putProvidersAuthCore({ provider: id });
  ...
  const initializeStep = async () => {
    ...
    try {
      ...
      setIsLoading(true);
      resetError();
      if (await checkAuthProviderCoreConfigured()) {
        setSetupMode('completed');
        await completeStep();
        return;
      }
      const r = await getProvidersAuth();
      const data = unwrapSuccess(r)!;
      setAuthProviders(data);
      const ids = getProviderIds(data);
      if (ids.length > 0) {
        setSelectedProvider(ids[0]);
        setSetupMode('select');
        if (ids.length === 1) await configureAndComplete(ids[0]);
      } else {
        setSetupMode('first-time');
        const fr = await postProvidersAuthFirstTimeSetupFlecsport();
        setSetupMode('polling');
        const questId = unwrapSuccess(fr)!.jobId;
        await fetchQuest(questId);
        const result = await waitForQuest(questId);
        if (!questStateFinishedOk(result.state)) throw new Error('Failed to import auth provider.');
        const r2 = await getProvidersAuth();
        const d2 = unwrapSuccess(r2)!;
        if (d2?.core) {
          setSetupMode('completed');
          await completeStep();
          return;
        }
        const ids2 = getProviderIds(d2);
        if (ids2.length > 0) {
          setAuthProviders(d2);
          setSelectedProvider(ids2[0]);
          setSetupMode('select');
          await configureAndComplete(ids2[0]);
        } else throw new Error('No providers found after setup');
      }
    } catch (err: unknown) {
  ...
  if (error || parentError)
    return (
      <div className="max-w-xl mx-auto">
        <h5 className="text-xl font-semibold mb-2">Auth Provider Setup Error</h5>
        <div className="px-4 py-3 rounded-lg bg-error/10 text-error mb-4">
          {error || parentError}
        </div>
  ...
  if (setupMode === 'loading')
    return (
      <div className="max-w-xl mx-auto text-center py-8">
        ...
        <p className="mt-4 text-sm">Checking authentication provider status...</p>
      </div>
    );
  if (setupMode === 'completed')
    return (
      <div className="max-w-xl mx-auto">
        <h5 className="text-xl font-semibold mb-2">Authentication Provider</h5>
        <div className="px-4 py-3 rounded-lg bg-success/10 text-success mb-4">
          Authentication provider is already configured.
        </div>
        <div className="flex justify-between mt-8">
          <button
            onClick={onPrevious}
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 text-center">
      ...
      <div className="px-4 py-3 rounded-lg bg-accent/10 text-accent mb-4">
        Authentication provider found. Confirm to proceed.
      </div>
      <div className="mb-6">
        <label className="block text-sm text-muted mb-1">Select Provider</label>
        <select
      ...
      <div className="flex justify-between mt-8">
        ...
        <button
          onClick={() => selectedProvider && configureAndComplete(selectedProvider)}
        ...
        >
          {isLoading ? 'Setting up...' : 'Setup Provider'}
        </button>
      </div>
    </div>
  );
