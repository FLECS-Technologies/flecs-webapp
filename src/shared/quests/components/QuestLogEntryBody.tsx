...
import { questStateFinishedOk } from '../utils/QuestState';
import { hasQuestFailedSubquest } from '../utils/Quest';
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', width: '100%', gap: 0.5 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        ...
        {quest.state !== QuestState.Failed && hasQuestFailedSubquest(quest) && (
          <AlertTriangle size={20} color="var(--mui-palette-warning-main)" />
        )}
      ...
      {quest.subquests && !questStateFinishedOk(quest.state) && (
        <SubQuestProgressIndicator subquests={quest.subquests} state={quest.state} />
      )}
    </Box>
  );
};
