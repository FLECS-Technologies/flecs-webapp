...
const isFinished = (s: QuestState) =>
  s !== QuestState.failing && s !== QuestState.ongoing && s !== QuestState.pending;
...
const hasFailedSub = (q: Quest): boolean =>
  q.subquests?.some(
    (s) => s.state === QuestState.failed || s.state === QuestState.failing || hasFailedSub(s),
  ) ?? false;
...
function QuestIcon({ state }: { state: QuestState }) {
  switch (state) {
    ...
    case QuestState.failing:
      return <AlertCircle size={18} className="text-error shrink-0" />;
    case QuestState.success:
      return <CheckCircle2 size={18} className="text-success shrink-0" />;
    case QuestState.pending:
      return <Hourglass size={18} className="opacity-50 shrink-0" />;
    case QuestState.skipped:
      return <Ban size={18} className="opacity-50 shrink-0" />;
    default:
      return (
  ...
  return (
    <div className="w-full mt-1">
      ...
      <div className="flex justify-between mt-0.5 text-[11px] text-muted">
        ...
        <span>{total - done - running} pending</span>
      </div>
    </div>
  );
}
    ...
    <div className={`ml-2 border-l-4 ${borderColor(quest.state)} pl-2 py-1`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {quest.state !== QuestState.failed && hasFailedSub(quest) && (
        ...
        {quest.progress && !isFinishedOk(quest.state) && (
          <Progress progress={quest.progress} state={quest.state} />
        )}
        {quest.subquests && !isFinishedOk(quest.state) && (
          <SubProgress subquests={quest.subquests} state={quest.state} />
        )}
      </div>
      {open && hasSubs && level < 10 && (
        <div className="mt-1">
          {quest.subquests!.map((sub) => (
            <QuestItem key={sub.id} id={sub.id} level={level + 1} />
          ))}
        </div>
      )}
    </div>
