...
import type { FetchError } from '@app/api/fetch-error';

function isLaterThan(a?: string, b?: string): boolean {
  if (!a || !b) return false;
...
async function fetchLatestGithubVersion(): Promise<{ version: string } | null> {
  try {
    const res = await fetch(
      'https://api.github.com/repos/FLECS-Technologies/flecs-public/releases/latest',
    );
    if (!res.ok) return null;
    const data = await res.json();
    const tag = data?.tag_name as string | undefined;
    return tag ? { version: tag.replace(/^v/, '') } : null;
  } catch {
    return null;
  }
}
...
export default function Version() {
  ...
  const systemInfo = unwrapSuccess(infoResponse);
  const {
  ...
  React.useEffect(() => {
    ...
    if (!loadingLatestVersion) fetchLatestVersion();
    executedRef.current = true;
  }, []);
  React.useEffect(() => {
    if (versionError) {
      setError(true);
      setErrorText((versionErrorObj as FetchError)?.message);
    }
  }, [versionError, versionErrorObj]);

  const fetchLatestVersion = async () => {
    setLoadingLatestVersion(true);
    fetchLatestGithubVersion()
      .then((r) => {
        if (r) setLatestVersion(r);
        setError(false);
      })
      .catch((e) => {
      ...
      .finally(() => setLoadingLatestVersion(false));
  };
  ...
  return (
    <div>
      <div>
        ...
        {(loadingVersion || loadingLatestVersion) && (
          <p className="text-center text-sm">Checking for updates...</p>
        )}
      </div>
      <div>
        {isLaterThan(latestVersion?.version, version?.core) && (
          <div className="px-4 py-3 rounded-lg bg-accent/10 text-accent mb-4">
            ...
            <VersionSelector
              availableVersions={[{ version: latestVersion!.version, installed: false }]}
              selectedVersion={{ version: latestVersion!.version, installed: false }}
              setSelectedVersion={() => {}}
            />
            <p className="text-sm">
