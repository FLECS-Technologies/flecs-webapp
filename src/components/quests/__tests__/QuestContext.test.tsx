...
import { QuestState, Quest } from 'core-client/api'
import { AxiosResponse } from 'axios'

// Consumer component to expose context values for testing
function TestConsumer() {
  const { quests, mainQuestIds, fetching, setFetching, fetchQuest, clearQuests } = useQuestContext(QuestContext)
...
describe('QuestContextProvider integration', () => {
  ...
  it('initial fetch populates mainQuestIds and questCount', async () => {
    ...
    vi.spyOn(api.quests, 'questsGet').mockResolvedValue({ data: mockQuests }as any as AxiosResponse<Quest[], any>)
  ...
  it('polling triggers additional fetches when fetching is true', async () => {
    const mockQuests: Quest[] = []
    const spy = vi.spyOn(api.quests, 'questsGet').mockResolvedValue({ data: mockQuests }as any as AxiosResponse<Quest[], any>)
  ...
  it('fetchQuest adds quest and subquests to map', async () => {
    ...
    const mainQuest: Quest = { id: 5, state: QuestState.Ongoing, subquests: [subQuest] } as Quest
    // Prevent initial fetch from adding quests
    vi.spyOn(api.quests, 'questsGet').mockResolvedValue({ data: [] } as any as AxiosResponse<Quest[], any>)
    vi.spyOn(api.quests, 'questsIdGet').mockResolvedValue({ data: mainQuest } as any as AxiosResponse<Quest, any>)
  ...
  it('clearQuests deletes only finished main quests', async () => {
    ...
    vi.spyOn(api.quests, 'questsGet').mockResolvedValue({ data: [finishedQuest, ongoingQuest] } as any as AxiosResponse<Quest[], any>)
    const deleteSpy = vi.spyOn(api.quests, 'questsIdDelete').mockResolvedValue({} as any as AxiosResponse<void, any>)
  ...
  it('useQuestContext throws if no provider', () => {
    ...
    expect(() => render(<TestConsumer />)).toThrow('useQuestContext must be used within a QuestProvider')
