...
interface PortWithProtocol { protocol: TransportProtocol; port: InstancePortMappingSingle | InstancePortMappingRange; _rowId: string; }
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-4">
        <AddSinglePortMappingButton onAdd={() => setPorts(prev => [...prev, { protocol: TransportProtocol.tcp, port: { host_port: 0, container_port: 0 } as InstancePortMappingSingle, _rowId: crypto.randomUUID() }])} defaultProtocol={TransportProtocol.tcp} />
        <AddPortRangeMappingButton onAdd={() => setPorts(prev => [...prev, { protocol: TransportProtocol.tcp, port: { host_ports: { start: 0, end: 0 }, container_ports: { start: 0, end: 0 } } as InstancePortMappingRange, _rowId: crypto.randomUUID() }])} defaultProtocol={TransportProtocol.tcp} />
      </div>
      <div>
        {ports.length === 0 && <p className="text-sm text-muted">No ports configured.</p>}
        {ports.map((p, index) => 'host_port' in p.port ? <SinglePortMapping key={p._rowId} port={p.port as InstancePortMappingSingle} protocol={p.protocol} index={index} onChange={handlePortChange} handleDeletePort={handleDeletePort} handleSavePort={handleSave} handleProtocolChange={handleProtocolChange} /> : <PortRangeMapping key={p._rowId} port={p.port as InstancePortMappingRange} protocol={p.protocol} index={index} onChange={handlePortChange} handleDeletePort={handleDeletePort} handleSavePort={handleSave} handleProtocolChange={handleProtocolChange} />)}
      </div>
    </div>
  );
};
