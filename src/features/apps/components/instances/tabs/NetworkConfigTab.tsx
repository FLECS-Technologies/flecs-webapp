...
import {
  useGetInstancesInstanceIdConfigNetworks,
  usePostInstancesInstanceIdConfigNetworks,
  useDeleteInstancesInstanceIdConfigNetworksNetworkId,
} from '@generated/core/instances/instances';
import { useGetSystemNetworkAdapters } from '@generated/core/system/system';
import {
  useGetDeploymentsDeploymentIdNetworks,
  usePostDeploymentsDeploymentIdNetworks,
  usePostDeploymentsDeploymentIdNetworksNetworkIdDhcpIpv4,
} from '@generated/core/deployments/deployments';
...
export interface NetworkState {
  ...
  networks: NetworkAdapter['networks'];
...
const NetworkConfigTab: React.FC<NetworkConfigTabProps> = ({ instanceId, onChange }) => {
  ...
  const { data: deploymentNetworksResponse, isLoading: loadingDeployments } =
    useGetDeploymentsDeploymentIdNetworks('default');
  const { data: instanceNetworksResponse, isLoading: loadingInstance } =
    useGetInstancesInstanceIdConfigNetworks(instanceId);
  ...
  const networks: NetworkState[] = useMemo(() => {
    ...
    const networkAdapters = adaptersData.map((a) => ({
      id: a.name,
      name: a.name,
      net_type: a.net_type,
    ...
    const deploymentNetworks = deploymentsData.map((n) => ({
      parent: n.parent || '',
      name: n.name || '',
      type: 'deployment',
    }));
    const instanceNetworks = instanceData.map((n) => ({
      name: n.name || '',
      ipAddress: n.ipAddress,
    }));
    return networkAdapters
      .filter((a) => a.net_type === NetworkType.wired || a.net_type === NetworkType.wireless)
      ...
      .map((adapter) => {
        ...
        return {
          ...adapter,
          deploymentNetworkName: dep?.name || '',
          ipAddress: inst?.ipAddress || '',
          is_activated: !!inst?.ipAddress,
        };
      });
      ...
      try {
        if (isActivated) {
          if (!deploymentNetworkName) {
            const n = `flecs-ipvlan_l2-${name}`;
            await createDeploymentNetwork({
              deploymentId: 'default',
              data: { network_id: n, network_kind: NetworkKind.ipvlanl2, parent_adapter: name },
            });
            deploymentNetworkName = n;
          }
          const ipRes = await reserveIp({
            deploymentId: 'default',
          ...
          await connectNetwork({
            instanceId,
            data: { network_id: deploymentNetworkName, ipAddress: ip },
          });
        } else {
          await disconnectNetwork({ instanceId, networkId: network.deploymentNetworkName });
        }
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-4">
        <h6 className="text-base font-semibold">Connect Network Interface to App</h6>
        <HelpButton url={instancenicconfig} />
      </div>
      <div>
        {networks.map((n) => (
          <NetworkConfigCard
