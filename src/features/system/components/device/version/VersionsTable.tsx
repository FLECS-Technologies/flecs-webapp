...
import { CoreVersion, Distro, Kernel } from '@shared/types/VersionInterfaces';
...
const VersionsTable: React.FC<VersionsTableProps> = ({
  ...
  const versions = React.useMemo(
    () => [
      ...
      createData('UI', webappVersion, sbomHref),
        ...
        {versions.map(({ component, version, sbom }) => (
          <TableRow key={`${component}-${version}`} style={{ borderBottom: 'none' }}>
            ...
            <TableCell style={{ borderBottom: 'none' }}>
              {sbom && (
                <Button href={sbom} target="_blank" endIcon={<ExternalLink size={16} />}>
                  SBOM
                </Button>
              )}
            </TableCell>
