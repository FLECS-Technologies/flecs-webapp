...
import { getAuthCoreProvider } from '@features/auth/utils/authprovider-utils';
...
const Export: React.FC<ExportProps> = (props) => {
  ...
  const exportApps = async () => {
    ...
    try {
      ...
      await fetchQuest(exportQuest.data.jobId);
      const result = await waitForQuest(exportQuest.data.jobId);
