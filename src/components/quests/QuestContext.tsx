...
export const useQuestContext = (QuestContext: React.Context<QuestContextType | undefined>): QuestContextType => {
...
const QuestContextProvider = ({ children }: { children: ReactNode }) => {
  ...
  const clearQuests = async () => {
    ...
    try {
      await Promise.all(
        finishedQuests.filter((quest) => mainQuestIds.includes(quest.id)).map((quest) =>
          api.quests.questsIdDelete({ id: quest.id })
        )
      );
    } catch (error) {
      console.error(error);
