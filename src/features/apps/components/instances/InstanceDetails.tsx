...
export default function InstanceDetails({ instance }: InstanceDetailsProps) {
  ...
  const [instanceDetails, setInstanceDetails] = React.useState<
    GetInstancesInstanceId200 | undefined
  >();

  React.useEffect(() => {
    if (executedRef.current) return;
    if (!loadingDetails) fetchDetails();
    if (reloadDetails) setReloadDetails(false);
  ...
  const fetchDetails = async () => {
    ...
    getInstancesInstanceId(instance.instanceId)
      .then((response) => {
        ...
        setNetworkDetails([
          { name: 'IP Address', info: data?.ipAddress ?? '' },
          { name: 'Hostname', info: data?.hostname ?? '' },
        ]);
  ...
  return (
    <div className="overflow-x-auto" aria-label="instance-details-container">
      {networkDetails.length > 0 && (
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {(instanceDetails?.ports?.length ?? 0) > 0 && (
              <CollapsableRow title="Ports">
                <HostContainerTable data={instanceDetails?.ports ?? []} />
              </CollapsableRow>
            )}
          </tbody>
        </table>
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {(instanceDetails?.volumes?.length ?? 0) > 0 && (
            <CollapsableRow title="Volumes">
              <VolumesTable volumes={instanceDetails?.volumes ?? []} />
            </CollapsableRow>
          )}
          {(instanceDetails?.configFiles?.length ?? 0) > 0 && (
            <CollapsableRow title="Configuration Files">
              <HostContainerTable data={instanceDetails?.configFiles ?? []} />
            </CollapsableRow>
          )}
        </tbody>
      </table>
