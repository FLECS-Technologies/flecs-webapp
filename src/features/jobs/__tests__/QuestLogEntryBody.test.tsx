...
import { QuestLogEntryBody } from '../components/QuestLogEntryBody';
import { QuestState, QuestProgress, Quest } from '@flecs/core-client-ts';
import * as QuestStateUtils from '../utils/QuestState';
import * as QuestUtils from '../utils/Quest';

// Mock Lucide AlertTriangle icon
vi.mock('lucide-react', () => ({
  __esModule: true,
  AlertTriangle: (props: any) => <div data-testid="warning-icon" {...props} />,
}));
// Mock child components using correct relative paths
vi.mock('../components/QuestIcon', () => ({
  __esModule: true,
  QuestIcon: ({ state }: { state: QuestState }) => <div data-testid="quest-icon">{state}</div>,
}));
vi.mock('../components/QuestProgressIndicator', () => ({
  __esModule: true,
  QuestProgressIndicator: () => <div data-testid="progress-indicator" />,
}));
vi.mock('../components/SubQuestProgressIndicator', () => ({
  __esModule: true,
