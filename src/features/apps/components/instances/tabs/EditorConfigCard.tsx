...
interface EditorConfigCardProps { editor: InstanceEditor; instanceId: string; }
const createCustomUrl = (editorUrl: string) => { const base = host(); if (!base) return editorUrl; return new URL(editorUrl, base).toString(); };

const EditorConfigCard: React.FC<EditorConfigCardProps> = ({ editor, instanceId }) => {
  ...
  const putEditorPrefix = async (port: number, pathPrefix: string) => { try { await putPrefix({ instanceId, port, data: { path_prefix: pathPrefix } }); setCurrentEditorPathPrefix(pathPrefix); toast.success('Editor prefix saved'); } catch (error) { toast.error('Failed to save editor prefix', { description: '' + error }); } };
  const deleteEditorPrefix = async (port: number) => { try { await deletePrefix({ instanceId, port }); setCurrentEditorPathPrefix(undefined); setEditorPathPrefix(''); toast.success('Editor prefix deleted'); } catch (error) { toast.error('Failed to delete editor prefix', { description: '' + error }); } };
