...
import {
  usePutInstancesInstanceIdConfigEditorsPortPathPrefix,
  useDeleteInstancesInstanceIdConfigEditorsPortPathPrefix,
...
const EditorConfigCard: React.FC<EditorConfigCardProps> = ({ editor, instanceId }) => {
  ...
  const putEditorPrefix = async (port: number, pathPrefix: string) => {
    try {
      await putPrefix({ instanceId, port, data: { path_prefix: pathPrefix } });
      setCurrentEditorPathPrefix(pathPrefix);
      toast.success('Editor prefix saved');
    } catch (error) {
      toast.error('Failed to save editor prefix', { description: '' + error });
    }
  };
  const deleteEditorPrefix = async (port: number) => {
    try {
      await deletePrefix({ instanceId, port });
      setCurrentEditorPathPrefix(undefined);
      setEditorPathPrefix('');
      toast.success('Editor prefix deleted');
    } catch (error) {
      toast.error('Failed to delete editor prefix', { description: '' + error });
    }
  };
  ...
  return (
    <div className="p-4 rounded-xl border border-border">
      ...
      <div className="mb-3">
        <p className="text-xs text-muted font-semibold mb-1">Fixed URL</p>
        <p className="text-xs font-mono text-muted break-all">{fixedUrl}</p>
      </div>
      ...
      {customUrl && (
        <div>
          <p className="text-xs text-muted font-semibold mb-1">Custom URL</p>
          <div className="flex items-center gap-2">
