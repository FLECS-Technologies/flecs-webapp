...
export default function Version() {
  ...
  React.useEffect(() => { if (versionError) { setError(true); setErrorText((versionErrorObj as FetchError)?.message); } }, [versionError, versionErrorObj]);
      ...
      <div>
        {isLaterThan(latestVersion?.version, version?.core) && (
          <div className="px-4 py-3 rounded-lg bg-accent/10 text-accent mb-4">
            ...
            <VersionSelector availableVersions={[{ version: latestVersion!.version, installed: false }]} selectedVersion={{ version: latestVersion!.version, installed: false }} setSelectedVersion={() => {}} />
