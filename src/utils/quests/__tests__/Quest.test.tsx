...
describe('Quest utility functions', () => {
  describe('hasQuestFailedSubquest', () => {
    ...
    it('returns false if no subquests have failed or failing', () => {
      ...
      const quest: Quest = { id: 1, state: QuestState.Pending, subquests: [sub1, sub2] } as Quest
      expect(hasQuestFailedSubquest(quest)).toBe(false)
    })

    it('returns true if an immediate subquest is Failed or Failing', () => {
      const failed: Quest = { id: 2, state: QuestState.Failed } as Quest
      const quest1: Quest = { id: 1, state: QuestState.Pending, subquests: [failed] } as Quest
      expect(hasQuestFailedSubquest(quest1)).toBe(true)

      const failing: Quest = { id: 3, state: QuestState.Failing } as Quest
      const quest2: Quest = { id: 1, state: QuestState.Pending, subquests: [failing] } as Quest
    ...
    it('returns true if a nested subquest has failed', () => {
      ...
      const child: Quest = { id: 3, state: QuestState.Pending, subquests: [deepFail] } as Quest
      const parent: Quest = { id: 1, state: QuestState.Pending, subquests: [child] } as Quest
    ...
    it('handles mixed nested structures correctly', () => {
      ...
      const child1: Quest = { id: 3, state: QuestState.Pending, subquests: [deepPending] } as Quest
      const child2: Quest = { id: 4, state: QuestState.Pending, subquests: [deepFailing] } as Quest
      const quest: Quest = { id: 1, state: QuestState.Pending, subquests: [child1, child2] } as Quest
